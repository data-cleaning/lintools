...
To find a solution, the successive projection algorithm (hence: `supral`)
...
### Optimization in R
In the `supral` package, all inequalities must be written in the $\leq$-form. So
...
So we formulate the problem with the `supral` package as follows.
```{r}
library(supral)
x <- c(0.8,-0.2)
...
In the `supral` package, the row-column-coefficient format is used. That is, in
...
on `supral` with guaranteed backward compatibility.
